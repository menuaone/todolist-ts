...
(function () {
  ...
  let todos: Todo[] = []; // обозначили типы по интерфейсу
  let users: User[] = []; // обозначили типы по интерфейсу
  ...
  function createUserOption(user: User) {
    if (userSelect) {
      ...
      option.value = String(user.id); // явно оборачиваем юзер айди в строку
